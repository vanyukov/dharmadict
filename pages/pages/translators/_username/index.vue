<template>
  <div>
    <div class="flex flex-row items-center">
      <button 
        @click="goBack()"
        class="transition duration-500 ease-in-out transform hover:-translate-y-0.5 hover:shadow-lg text-white w-10 h-10 p-3 m-3 font-bold rounded-full inline-flex items-center justify-center focus:outline-none bg-green-500"
...
export default {
  async asyncData({ params, $api, error }) {
    try {
      const data = await $api('v1', `translator/${params.username}/terms?page=1&per_page=1`)
      
      const about = data.translator.page ? await $api('v1', `page/${data.translator.page}`) : {content: ''}
      return { data, about }
    } catch (e) {
      return error({
        statusCode: 404,
