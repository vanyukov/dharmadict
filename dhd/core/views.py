...
def index(request):
    return HttpResponse(
        "Буддийская терминология в русских переводах. Версия 2 – в процессе разработки."
    )
...
def edit_user(request, user_id):
    ...
    return render(
        request,
        "user.html",
        context=common_context(request, {"profile": user, "is_self": is_self,}),
    )
...
def new_user(request):
    ...
    return render(
        ...
        context=common_context(
            request, {"profile": None, "new_user": True, "is_self": False,}
        ),
    )
...
def api_terms(request):
    t = (
        TermFilter(request.GET, Term.objects.all())
        if ("search" in request.GET and request.GET["search"] != "")
        else TermFilter(request.GET, Term.objects.none())
    )
    m = (
        MeaningFilter(request.GET, Meaning.objects.all())
        if ("search" in request.GET and request.GET["search"] != "")
        else MeaningFilter(request.GET, Meaning.objects.none())
    )

    res = {}
    for t in t.qs:
        if not t.wylie in res and t.rank > 0.0001:
...
def api_translator_terms(request, username):
    terms_qs = Term.objects.all().prefetch_related(
        Prefetch(
            "meanings",
            queryset=CustomUser.translators().get(username=username).meanings.all(),
        )
    )
    ...
    res = {
        "translator": CustomUser.translators().get(username=username).json(),
        "count": len(qs),
        "page": page,
        "per_page": per_page,
        "terms": {},
...
def user(request):
    ...
    return render(
        request,
        "user.html",
        context=common_context(request, {"profile": u, "is_self": True,}),
    )
...
def users_export(request):
    response = HttpResponse(
        CustomUserResource().export().csv, content_type="text/csv; charset=utf-8"
    )
...
def paginate_users(request):
    ...
    return render(
        request,
        "paginators/users.html",
        context=common_context(
            request,
            paginator_context(
                request,
                UserFilter(request.GET, active_users()),
                target="users",
                header=True,
            ),
        ),
